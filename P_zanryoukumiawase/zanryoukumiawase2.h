void driveStart_zancom2();
void get_combi_location_zancom2(combiData_t com[], combiData_t com2[], combiData_t com3[], int *secondCnt);
void show_results_zancom2(const combiData_t *com);
...
void driveStart_zancom2()
{
    ...
    //******親ヘッドの状況確認start*********************************
        ...
        for(int i=0; i<product.head; i++){
            if(cel.anteiWeight(i) > param.CUT){ //CUT以下
                befour_oya_weight = int(cel.anteiWeight(i));
            ...
            if(cel.anteiWeight(i) < param.CUT)  {
            ...
            }else if(cel.anteiWeight(i) < param.CUT){ //CUT以下
               ...
               // lcd.bigNumber(int(cel.anteiWeight(i))); //大文字で重量を表示
            }
            ...
            if(cel.anteiWeight(i) < param.CUT){ //CUT以下
                go_combi_flg[i]=0;
                if(nowweightVale[i] < param.CUT_Y){
                    if((light_combi & (1 << i)) > 0){ //光らせてた奴が取られた
                        printf("%2d:",light_combi);
                        light_combi = light_combi ^ (1 << i);
                        printf("%2d\r\n",light_combi);
                        if(light_combi <= 0){
                            se.Touch_01();
                            se.Touch_01();
                            ...
                            coulap = 0;
                            total_kumi_cnt += 1;
                            kumi_value_data = float(int(light_value_save*10))/10;
                            average_kumi_value = (((average_kumi_value * (total_kumi_cnt -1)) + kumi_value_data) / total_kumi_cnt);
                            bit_num = 1;    //成立した個数の保存
                            bit_place = light_combi_save;
                            add_combi_end();
                            save_weight[i] = 0;
                            light_combi_save = 0;
                            light_value_save = 0.0;
                        }
                    }
                    if(save_weight[i] > 0 ){
                        se.Touch_01();
                        ...
                        coulap = 0;
                        total_kumi_cnt += 1;
                        kumi_value_data = float(int(save_weight[i]*10))/10;
                        average_kumi_value = (((average_kumi_value * (total_kumi_cnt -1)) + kumi_value_data) / total_kumi_cnt);
                        bit_num = 1;    //成立した個数の保存
                        bit_place = 1<<i;
                        add_combi_end();
            ...
            }else if(load.target < cel.anteiWeight(i) && cel.anteiWeight(i) < load.target + load.jougen/* && light_combi == 0*/){ ///目標内
                iro[i] = C_MARU;
                go_combi_flg[i]=2;
                save_weight[i] = cel.anteiWeight(i);
        ...
        for(int i=0; i<product.head; i++){
            if(go_combi_flg[0] != 2 && go_combi_flg[i] == 2){
                go_combi_flg[0] = 2;
            }else if(go_combi_flg[0] == 0 && (go_combi_flg[i] == 1 || go_combi_flg[i] == 3)){
                go_combi_flg[0] = go_combi_flg[i];
            }
            
            //printf("%2d:%2d:%2d\r\n",i,go_combi_flg[i],go_combi_flg[0]);
        }
        //go_combi_flg[0]=1;
    ...
    //******親ヘッドの状況確認end****************************************
        ...
        if(go_combi_flg[0] == 2){
            light_combi=0;
        }else if((go_combi_flg[0] == 0 || go_combi_flg[0] == 1 ) && light_combi == 0){
            get_combi_location_zancom2(orderCombi, orderCombi2, orderCombi3, &secondCnt);     //組み合わせ確認,組み合わせ候補を抽出
            ...
            for(int i = 0; i <= product.head ; i += 1) {
                ...
                if((resultCombi.kumi & (1 << i)) > 0){
                    ...
                    if(cel.anteiWeight(i) < param.CUT){ //CUT以下
                        ...
                        iro[i] = CR_01 +((CG_01-CR_01) * option.number_color) - 1 + (load.target - int(cel.anteiWeight(i)))%100;
                        //if(iro[i] < CR_01){iro[i] = CR_01;}
                        ...
                        iro[i] = CG_01 -((CG_01-CR_01) * option.number_color)  - 1  + (int(cel.anteiWeight(i) - (load.target + load.jougen)))%100;
                    }else{  //目標以上
                        iro[i] = C_NULL;
                    }
                }
    ...
    for(int i=in; i; i--) {
        ...
        for(bit = 0; bk; bk >>= 1) {
            if (bk & 0x01) {
                ...
                if((load.kumi_flag == KUMI_OYAKO || load.kumi_flag == KUMI_ZANCOM2)
    ...
    if(load.kumi_flag == KUMI_KAIKYU && (product.productType %100 == MATRIX || product.matrix_mode == MATRIX_ON) && int(com->total) > load.target && int(com->total) < res) {
        ...
        if(load.spMode == SPEED_HIGH) {
            if(option.kokuti==1||option.kokuti==2||option.kokuti==3){
                if(load.kumi_flag == KUMI_ZANCOM2){}
                else{                se.Touch_02();}
            }
            if(load.kumi_flag == KUMI_ZANCOM2){
                for(int i = 0; i <= product.head ; i += 1) {
